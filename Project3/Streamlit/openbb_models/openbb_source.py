...
def get_news(
    ...
    have_data = False
    if display_message:
        print("[yellow]Fetching data. Please be patient\n[/yellow]")
    n = 0
    ...
    while not have_data:
        ...
        if (
            ...
            if data.entries:
                ...
                print("[red]Timeout occurred. Please try again\n[/red]")
                break
            n += 1

        elif hasattr(data, "status") and data.status != 200:  # If data request failed
            print("[red]Status code not 200. Unable to retrieve data\n[/red]")
            break
        else:
            print("[red]Could not retrieve data\n[/red]")
            break
